...
export default class DocumentPackingSlipSettingsService extends TransactionBaseService {
  private copySettingsIfPossible(
    newSettings: DocumentPackingSlipSettings,
    lastSettings?: DocumentPackingSlipSettings
  ) {
    if (lastSettings) {
  ...
  async getPackingSlipForcedNumber(): Promise<string | undefined> {
    const lastDocumentPackingSlipSettings =
      await this.getLastDocumentPackingSlipSettings();
    if (
      lastDocumentPackingSlipSettings &&
      lastDocumentPackingSlipSettings.forced_number
    ) {
      const nextNumber: string =
        lastDocumentPackingSlipSettings.forced_number.toString();
      return nextNumber;
    }
    return undefined;
  }
  ...
  async resetForcedNumberByCreatingNewSettings(): Promise<DocumentPackingSlipSettings> {
    const documentPackingSlipSettingsRepository =
      this.activeManager_.getRepository(DocumentPackingSlipSettings);
    const newDocumentPackingSlipSettings = this.activeManager_.create(
    ...
    const result = await documentPackingSlipSettingsRepository.save(
      newDocumentPackingSlipSettings
    );
    return result;
  }

  async getLastDocumentPackingSlipSettings(): Promise<
    DocumentPackingSlipSettings | undefined
  > {
    const documentPackingSlipSettingsRepository =
      this.activeManager_.getRepository(DocumentPackingSlipSettings);
    const lastDocumentPackingSlipSettings: DocumentPackingSlipSettings | null =
      await documentPackingSlipSettingsRepository
  ...
  async updatePackingSlipForcedNumber(
    forcedNumber: string | undefined
  ): Promise<DocumentPackingSlipSettings | undefined> {
  ...
  async updatePackingSlipTemplate(
    packingSlipTemplate: PackingSlipTemplateKind | undefined
  ): Promise<DocumentPackingSlipSettings | undefined> {
    const documentPackingSlipSettingsRepository =
      this.activeManager_.getRepository(DocumentPackingSlipSettings);
    const lastDocumentPackingSlipSettings =
