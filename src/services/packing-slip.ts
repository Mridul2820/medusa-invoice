...
} from "./generators/packing-slip-generator";
...
export default class PackingSlipService extends TransactionBaseService {
  ...
  private calculateFormatNumber(
    documentPackingSlipSettings: DocumentPackingSlipSettings
  ): string | undefined {
    if (
  ...
  async getPackingSlip(
    ...
    if (includeBuffer) {
      const packingSlip: PackingSlip | null = await this.activeManager_
        ...
        .leftJoinAndSelect(
          "packslip.document_packing_slip_settings",
      ...
      if (
        ...
        const calculatedTemplateKind = this.calculateTemplateKind(
          packingSlip.document_packing_slip_settings
        );
  ...
  async create(orderId: string): Promise<PackingSlipResult> {
    ...
    if (order) {
      ...
      if (settings) {
        const packingSlipSettings: DocumentPackingSlipSettings | undefined =
          await this.documentPackingSlipSettingsService.getLastDocumentPackingSlipSettings();
        if (packingSlipSettings) {
          const calculatedTemplateKind =
            this.calculateTemplateKind(packingSlipSettings);
          ...
          if (validationPassed) {
            ...
            const packingSlipFormatNumber =
              this.calculateFormatNumber(packingSlipSettings);

            newEntry.display_number = packingSlipFormatNumber
              ? packingSlipFormatNumber.replace(
